...
namespace publicLibrary
{
    class DbWorkers : DbMain
    {
        ...
        {
            DataSet ds = new DataSet();
            string sql = string.Format("SELECT * FROM Workers WHERE workerName='{0}'", name);
            ds = GetQuery(sql);
            return ds;
        }
        ...
        public string[] LogIn(string password)
        {
            ...
            {
                userInfo[0] = (string)ds.Tables[0].Rows[0][1];
                userInfo[1] = password;
                userInfo[2] = (string)ds.Tables[0].Rows[0][3];
                userInfo[3] = ds.Tables[0].Rows[0][0].ToString();
